...
private:
    ...
    std::array<double, n_in>  input_in;
    std::array<double, n_hid> input_hid;
    std::array<double, n_out> input_out;

    /* それぞれの層に対するoutputを計算する配列 */
    std::array<double, n_in>  output_in;
    std::array<double, n_hid> output_hid;
    std::array<double, n_out> output_out;

    /* 隠れ層と出力層に対する誤差を計算する配列 */
    std::array<double, n_hid> error_hid;
    std::array<double, n_out> error_out;

    /* weight_'to'[from][to] で、fromからtoへの重みを格納 */
    std::array<std::array<double, n_hid> , n_in> weight_hid;
    std::array<std::array<double, n_out> , n_hid> weight_out;
